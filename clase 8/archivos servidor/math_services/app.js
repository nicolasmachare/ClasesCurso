...
app.post('/minus', function (request, response) {
...
app.put('/mult', function (request, response) {
    var form = new formidable.IncomingForm();
    form.parse(request, function (err, fields) {
        var x = Number(fields.x),
        y = Number(fields.y);
...
app.delete('/div', function (request, response) {
    ...
    form.parse(request, function (err, fields) {
        ...
        if (y != 0) {
            ...
            response.writeHead(200, { 'Content-Type': 'application/json' });
            response.end('{ "result": ' + result + '}');
            console.log('Handled error request for x=' + x + ' : y=' + y);
        }
